...
def masked_mse(
    ...
    Args:
        ...
        masks: Optional List of binary tf.Tensor objects with the same shape
            as Tensors in comparison_list. Each mask Tensor defines the elements to be
            used for mse calculation.  If the mask at a location is 1, then
    ...
    differences = tf.transpose(tf.matrix_band_part(tf.transpose(differences, (2, 0, 1)), -1, 0), (1, 2, 0))
...
def warp_loss(dense_warp, correlation_range):
    ...
    center_replacement = tf.pad(tf.ones([1, 1]),
                                [[correlation_range, correlation_range], [correlation_range, correlation_range]])
    correlation_kernel = ham2d - center_replacement * tf.reduce_sum(ham2d)
    correlation_kernel = correlation_kernel / correlation_kernel[correlation_range, correlation_range]
    correlation_kernel = tf.stack([correlation_kernel, tf.zeros_like(correlation_kernel)], axis=2)
    correlation_kernel = tf.stack([correlation_kernel, correlation_kernel[:, :, ::-1]], axis=3)
